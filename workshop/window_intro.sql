...
# ile klient wydal miesiecznie + roznica wzgledem poprzedniego misiaca
...
SELECT cust.customer_id,
       ...
       SUM(pt.amount) - LAG(SUM(pt.amount))
                            OVER (PARTITION BY cust.customer_id
                                ORDER BY DATE_FORMAT(pt.payment_date, '%Y-%m')) AS diff_from_prev_month
FROM customer cust
         JOIN payment pt ON cust.customer_id = pt.customer_id
...
# top 3 filmy z kazdej kategorii (cartegory_name, film_title, rental_count, ranking)
...
SELECT CASE
           ...
           WHEN avg_film_rate > 2 THEN 'superb acting'
           ELSE 'error' END AS actingLevel,
       COUNT(actor_id)      AS countActor,
       SUM(actor_payload),
       SUM(films_amount),
       AVG(avg_film_rate)
...
SELECT *,
       RANK() OVER (ORDER BY avg_film_rate DESC) AS rank_actors_avg_rate
FROM actor_analytics
LIMIT 500;
...
SELECT actor_id,
       ...
       (ROW_NUMBER() OVER (payload))                    AS count_percent_2,
       COUNT(1) OVER ()                                 AS count_percent_3,
       SUM(actor_payload) OVER (payload) / SUM(actor_payload) OVER ()
FROM actor_analytics
WINDOW payload AS (ORDER BY actor_payload DESC);
